...
export function MonthMobile({
    ...
    useEffect(() => {
        const updateDayWidth = () => {
            if (containerRef.current) {
                ...
                setDayWidth(Math.max(calculatedDayWidth, 160)); // минимум 150px
            }
        };
    ...
    return (
        ...
        >
            ...
            <div
                className={clsx(
                    "absolute font-medium bg-default-50 bottom-3 right-2 flex flex-col",
                    "justify-end text-center w-[150px] text-[12px] z-10 p-2",
                    "rounded border border-default-300 shadow"
                )}>

                    <>
            <p>Встреча с {nextMeetingInfo.title}</p>
            <p>
            через <span className={clsx(
                "font-semibold",
                nextMeetingInfo.minutesLeft <= 15 && "text-danger",
                nextMeetingInfo.minutesLeft > 15 && nextMeetingInfo.minutesLeft <= 60 && "text-warning",
                nextMeetingInfo.minutesLeft > 60 && "text-default-600"
            )}>{nextMeetingInfo.formattedTime}</span>!</p>
            </>
