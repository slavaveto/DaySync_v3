...
export function QuickNotesProjects({ height, project }: QuickNotesProps) {
    const { items, setItems, setHasLocalChanges, setIsUserActive } = useMainContext();
    const { user } = useUser();
    const [topPanelText, setTopPanelText] = usePersistentState(`rightPanelTopText_${project}`, '');
    const [initialQuickNoteText, setInitialQuickNoteText] = useState('');
    const localRef = useRef<HTMLDivElement | null>(null);

    // Маппинг проектов на фиксированные ID для quick_notes
    const PROJECT_QUICK_NOTES_IDS: Record<string, number> = {
        daysync: 999999991,
        psyhelp: 999999992,
    ...
    const QUICK_NOTES_ID = PROJECT_QUICK_NOTES_IDS[project] || 999999991; // fallback ID
    ...
    useEffect(() => {
        ...
        if (quickNote?.notes) {
            setTopPanelText(quickNote.notes);
            setInitialQuickNoteText(quickNote.notes);
        } else {
            // Если заметки для этого проекта нет, очищаем поля
            setTopPanelText('');
            setInitialQuickNoteText('');
        }
    }, [items, setTopPanelText, QUICK_NOTES_ID]);
...
// Функция для сброса и установки нового таймера
    ...
    return (
        ...
        >
            ...
            {/*<p className={clsx(*/}
            {/*    "mx-[6px] h-[30px] py-2 font-semibold text-[14px]",*/}
            ...
            <div
                ...
                className={clsx(
                    "w-full resize-none leading-[20px] p-[6px]  outline-none border-none text-[14px] text-default-700",
                    " focus:bg-primary-50/50 resize-none overflow-y-auto"
                )}
                style={{
                    height: `${height - 30}px`,
                    fontFamily: 'inherit'
