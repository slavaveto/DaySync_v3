...
export const Fixed = ({
                          groupedClients, selectedMonth, tableWidth = 600, exchangeRate,
                          dbStatus, isLocked
                      }: ClientsInfoProps) => {
    const {clients, setClients, items, tabs, subtabs,} = useMainContext();

    const [fixedUploadedStatuses, setFixedUploadedStatuses] = usePersistentState<{
        [key: string]: boolean
    }>('fixedUploadedStatuses', {});
    const getFixedUploadedStatus = (clientId: number, month: string) => {
        return fixedUploadedStatuses[`${clientId}-${month}`] || false;
    };
    const setFixedUploadedStatus = (clientId: number, month: string, isUploaded: boolean) => {
        setFixedUploadedStatuses(prev => ({
            ...prev,
            [`${clientId}-${month}`]: isUploaded
        }));
    };
    ...
    const DYNAMIC_COLUMNS = isPastMonth ? [
        ...
        {key: 'all_meetings', width: 50, align: 'center' as const},
        {key: 'price_usd', width: 100, align: 'center' as const},
        {key: 'sum', width: 100, align: 'center' as const},
        // {key: 'paid_meetings', width: 60, align: 'center' as const},
        // {key: 'debt', width: 100, align: 'center' as const},
        // {key: 'fixed', width: 60, align: 'center' as const},
        {key: 'fixed_uploaded', width: 60, align: 'center' as const},

        // {key: 'fixed_amount', width: 100, align: 'center' as const},
        {key: 'fixed_name', width: 200, align: 'left' as const},
    ] : [
        {key: 'name', width: 230, align: 'left' as const},
        // {key: 'price', width: 100, align: 'center' as const},
        {key: 'price_usd', width: 100, align: 'center' as const},
        {key: 'past_meetings', width: 50, align: 'center' as const},
        {key: 'past_meetings_paid', width: 100, align: 'center' as const},
        {key: 'all_meetings', width: 50, align: 'center' as const},
        {key: 'sum', width: 100, align: 'center' as const},
        {key: 'historical_debt', width: 200, align: 'center' as const},
    ];
    ...
    const getTotalSum = () => {
        return Object.values(groupedClients)
            ...
            .reduce((total, client) => {
                ...
                const priceInUSD = roundPrice(client.price, client.currency, exchangeRate);
    ...
    const getHistoricalDebt = (clientId: number) => {
        ...
        // Проходим по всем прошлым месяцам (например, за последний год)
        for (let i = 1; i <= 12; i++) {
            ...
            if (pastDate >= new Date(currentDate.getFullYear(), currentDate.getMonth(), 1)) continue;
            ...
            const monthDebtUSD = roundPrice(monthDebt, client.currency, exchangeRate);
    ...
    const getGlobalNextFixedUnuploadedIndex = () => {
        // Собираем всех видимых fixed клиентов по всем дням в правильном порядке
        const allVisibleFixedClients: {client: ClientType, dayName: string}[] = [];

        Object.entries(groupedClients)
            .filter(([dayName, dayClients]) => dayClients.some(isVisibleClient))
            .forEach(([dayName, dayClients]) => {
                dayClients
                    .filter(isVisibleClient)
                    .filter(client => getFixedStatus(client.id, selectedMonth))
                    .forEach(client => {
                        allVisibleFixedClients.push({client, dayName});
                    });
            });

        if (allVisibleFixedClients.length === 0) return null;

        // Находим последний отмеченный индекс подряд
        let lastUploadedIndex = -1;
        for (let i = 0; i < allVisibleFixedClients.length; i++) {
            if (getFixedUploadedStatus(allVisibleFixedClients[i].client.id, selectedMonth)) {
                if (i === lastUploadedIndex + 1) {
                    lastUploadedIndex = i;
                } else {
                    break;
                }
        ...
        if (lastUploadedIndex === -1 || lastUploadedIndex === allVisibleFixedClients.length - 1) {
            return null;
        }

        // Возвращаем следующего клиента
        return allVisibleFixedClients[lastUploadedIndex + 1];
    };

    // После определения isPastMonth
    if (!isPastMonth) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    <h2 className="text-xl text-gray-600 mb-2">Данные для текущего месяца недоступны</h2>
                    <p className="text-sm text-gray-500">Информация будет доступна после завершения месяца</p>
                </div>
            </div>
        );
    }
    ...
    return (
        ...
        <div className="space-y-0" style={{width: `${tableWidth}px`}}>
            ...
            <div
                ...
                style={{
                    marginLeft: `${nameColumnWidth + 50 + 100}px` // +50 для all_meetings, +100 для price_usd
                }}
            >
                <div className="relative flex h-[30px] items-center text-[14px] font-semibold">
                    ...
                    {/* Колонки с итогами */}
                    {columnPositions
                        .filter(col => col.key !== 'all_meetings' && col.key !== 'price_usd')
                        .map((col, index) => (
                        <div
                            key={col.key}
                            className={clsx(
                        ...
                        >
                            ...
                            {col.key === 'fixed_uploaded' && (() => {
                                const checkedCount = Object.values(groupedClients)
                                    .flat()
                                    .filter(isVisibleClient)
                                    .filter(client => getFixedStatus(client.id, selectedMonth))
                                    .filter(client => getFixedUploadedStatus(client.id, selectedMonth))
                                    .length;

                                const totalCount = Object.values(groupedClients)
                                    .flat()
                                    .filter(isVisibleClient)
                                    .filter(client => getFixedStatus(client.id, selectedMonth))
                                    .length;
            ...
            {Object.entries(groupedClients)
                .filter(([dayName, dayClients]) =>
                    dayClients.some(client =>
                        isVisibleClient(client) && getFixedStatus(client.id, selectedMonth)
                    )
                )
                .map(([dayName, dayClients]) => {
                    let visibleRowIndex = 0;
                    const globalNext = getGlobalNextFixedUnuploadedIndex();
                            ...
                            <div className={`${dayName === "Вторник" || dayName === "Четверг" ? "ml-[30px]" : ""}`}>
                                <div className="rounded-md overflow-hidden border border-default-300 mb-0">
                                    {dayClients
                                        .filter(isVisibleClient)
                                        .filter(client => getFixedStatus(client.id, selectedMonth))
                                        .map((client, localIndex) => {
                                            const globalIndex = allVisibleClients.findIndex(item => item.client.id === client.id);
                                            const currentVisibleIndex = visibleRowIndex++;
                                            const shouldHighlight = globalNext && globalNext.client.id === client.id;

                                            return (
                                                <div
                                                    key={client.id}
                                                    className={clsx(
                                                        "relative group flex h-[26px] cursor-default items-center last:border-0 border-b border-default-300 hover:bg-primary-50 transition-all duration-200 text-[14px]",
                                                        // currentVisibleIndex % 2 === 1 && "bg-primary-50/70",
                                                    // globalIndex % 2 === 1 && "bg-primary-50/70",
                                                        getFixedUploadedStatus(client.id, selectedMonth) && "opacity-50",
                                                        shouldHighlight && "!bg-danger-100 !font-semibold "
                                                    )}
                                                >
                                                    ...
                                                    {columnPositions.map((col, index) => (
                                                        ...
                                                        >
                                                            ...
                                                            {col.key === 'past_meetings_paid' && client.pay_per_session && (() => {
                                                                ...
                                                                const priceInUSD = roundPrice(totalPrice, client.currency, exchangeRate);
                                                                return `${priceInUSD} $`;
                                                            })()}
                                                            ...
                                                            {col.key === 'sum' && (() => {
                                                                if (!isPastMonth && client.exclude_from_calculations) return '';
                                                                if (!getFixedStatus(client.id, selectedMonth)) return ''; // добавить эту строку
                                                                const meetingsCount = getAllMeetingsCount(client.id);
                                                                const priceInUSD = roundPrice(client.price, client.currency, exchangeRate);
                                                                return `${priceInUSD * meetingsCount} $`;
                                                            })()}
                                                            ...
                                                            {col.key === 'paid_meetings' && (
                                                                <select
                                                                    ...
                                                                    className="w-full text-center text-sm bg-transparent outline-none focus:outline-none"
                                                                >
        ...
        {client.fixed_name && getAllMeetingsCount(client.id) > 0 ? (
            <input
                type="checkbox"
                disabled={isLocked}
                checked={getFixedUploadedStatus(client.id, selectedMonth)}
                onChange={(e) => setFixedUploadedStatus(client.id, selectedMonth, e.target.checked)}
                                                                ...
                                                                <div
                                                                    onClick={() => {
                                                                        if (getFixedStatus(client.id, selectedMonth)) {
                                                                            ...
                                                                            const priceUsd = roundPrice(client.price, client.currency, exchangeRate);
                                                                            const meetingsCount = getAllMeetingsCount(client.id);
