...
const {createTodayWindow} = require('./electron/winToday');

const {trayIconCountHandler} = require('./electron/iconTray');
const {dockIconHandler} = require('./electron/iconDock');

let mainWindow, todayWindow

const {setupPowerMonitor} = require("./electron/powerMonitor");
...
const SETTINGS_PATH = path.join(app.getPath("userData"), "daysync-window-settings.json");
...
function createTray() {
    const iconPath = path.resolve(__dirname, "electron", "icons", "tray_icon@2x.png");
